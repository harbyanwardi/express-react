...
const FiturAll = () => {
  ...
  const getfitur = async () => {
    try {
      ...
      // Map user data to an object for quick lookups
      const userMap = {};
      userData.forEach((user) => {
        userMap[user.id] = user.name;
      });

      // Add user names to feedback data
      const enrichedFeedbackData = feedbackData.map((item) => ({
        ...item,
        user_name: userMap[item.user_id] || "Unknown User",
      }));

      setfitur(enrichedFeedbackData);
    } catch (error) {
  ...
  let counterid = 1;
  const datatabel = fitur.map((item) => ({
    id: counterid++,
  ...
  const columns = [
    ...
    { name: "Feedback", selector: "isi", sortable: true },
  ];
        ...
        <div className="px-6 py-8">
          <div className="max-w-4xl">
            <div className="bg-white rounded-3xl p-8 mb-5">
              <h1 className="text-3xl font-bold mb-10">All Data For this Fitur</h1>
              <hr className="my-10" />

              <div className="h-full mt-14 mb-10">
                <section className="p-8">
                  <h1 className="pb-3 font-semibold text-xl text-gray-900">Fitur List</h1>
